...
import { collection, query, getDocs, addDoc, deleteDoc, orderBy, limit, Timestamp } from 'firebase/firestore'
...
export async function fetchArticles(pageNum) {
  const snapshot = await getDocs(query(collection(db, 'articles'), orderBy('date', 'desc'), limit((pageNum + 1) * 10)))
