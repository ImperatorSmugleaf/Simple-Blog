export default function Nav({ articles, setArticle, pageNum, setPageNum }) {
  ...
  return (
    <nav>
      ...
      <button onClick={() => setPageNum(pageNum - 1)} className="pageNav" disabled={pageNum === 0}>
        Previous Page
      </button>
      <p className="pageNav">{pageNum + 1}</p>
      <button onClick={() => setPageNum(pageNum + 1)} className="pageNav">
        Next Page
      </button>
    </nav>
  )
